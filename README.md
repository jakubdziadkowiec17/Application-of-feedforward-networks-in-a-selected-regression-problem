# Zastosowanie sieci feedforward w wybranym problemie regresyjnym

## Wybrany problem regresyjny: Prognoza cen domów w Bostonie
...
## Identyfikacja outlinerów przy użyciu z-score
```bash
import numpy as np
...
## Charakterystyka zmiennej wyjściowej
```bash
sns.histplot(medv)
...
### Wykres rzeczywistych i prognozowanych wartości
```bash
...
### Zestawienie rzeczywistych i prognozowanych wartości dla pierwszych 10 wierszy
```bash
...
### Optymalizacja metaparametrów i regresja za pomocą sieci neuronowej
```bash
...
### Wybór najlepszego modelu i prognozowanie
```bash
